...
def build_huffman_tree(freq_dict: dict[int, int]) -> HuffmanTree:
    ...
    if len(freq_dict) == 1:
        for i in freq_dict:
            if i < 255:
                h = HuffmanTree(None, HuffmanTree(i), HuffmanTree(i + 1))
            else:
                h = HuffmanTree(None, HuffmanTree(0), HuffmanTree(i))
            h.number = freq_dict[i]
            return h
        ...
        while len(lst) >= 2:
            ...
            lst.remove(min1)
            lst.remove(min2)
            if isinstance(min1[0], HuffmanTree) and isinstance(min1[0], HuffmanTree):
                ...
                h = HuffmanTree(None, HuffmanTree(min1[0]), HuffmanTree(min2[0]))
