...
class TermContract(Contract):
    ...
    def cancel_contract(self) -> float:
        ...
        """
        self.start = None
        if not ((self.month < self.end.month and self.year <= self.end.year)
                or self.year < self.end.year):
            self.bill.add_fixed_cost(-TERM_DEPOSIT)
        self.end = None
        return self.bill.get_cost()
...
class PrepaidContract(Contract):
    ...
    def new_month(self, month: int, year: int, bill: Bill) -> None:
        ...
        self.bill = bill
        if (month >= self.start.month and year == self.start.year) or year > \
                self.start.year:
    ...
    def cancel_contract(self) -> float:
        ...
        exists for the right month+year when the cancelation is requested.
        """
        if self.balance + self.bill.get_cost() > 0:
            return Contract.cancel_contract(self)
        else:
            self.balance = 0
            return 0
