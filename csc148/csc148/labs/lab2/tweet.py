...
class User:
    ...
    def retweet(new_user: str, tweet: Tweet, new_date: date) -> Tweet:
        ...
        >>> t1.likes
        1
        >>> t2 = User.retweet('Diane', t1, date(2017, 8, 20))
        >>> t2.content
        'David is so cool!'
        >>> t2.userid
        'Diane'
        ...
        new = Tweet(new_user, new_date, tweet.content)
        new.likes = 0
        return new
    ...
    def hack(self) -> None:
        ...
        for user in self.follows:
            for i in range(len(user.tweets)):
                user.tweets[i].content = 'mwahahaha'
